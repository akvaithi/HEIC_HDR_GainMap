...
all:
	...
	xcrun -sdk macosx metal -fcikernel CustomFilter/RGBGainMapKernel.ci.metal -o $(PLUGIN_DIR)/RGBGainMapKernel.ci.metallib
	# Compile Swift for x86_64
	swiftc -O -target x86_64-apple-macos15 \
		main.swift CustomFilter/*.swift Resource/*.swift -o $(BIN_X86)
	# Compile Swift for arm64
	swiftc -O -target arm64-apple-macos15 \
		main.swift CustomFilter/*.swift Resource/*.swift -o $(BIN_ARM)
...
clean:
	rm -rf $(BUILD_DIR)
	rm -f $(PLUGIN_DIR)/GainMapKernel.ci.metallib $(PLUGIN_DIR)/RGBGainMapKernel.ci.metallib $(BIN_UNI)
